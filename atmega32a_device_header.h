/*
 * atmega32a_device_header.h
 *
 * Created: 7/3/2022 7:16:28 AM
 *  Author: momen
 */ 


#ifndef ATMEGA32A_DEVICE_HEADER_H_
#define ATMEGA32A_DEVICE_HEADER_H_

//-----------------------------------------------------------------------------------
//includes
//-----------------------------------------------------------------------------------

#include <stdint.h>
#include <stdio.h>

//===================================================================================

//-----------------------------------------------------------------------------------
//Base addresses for peripherals
//-----------------------------------------------------------------------------------

#define GPIOA_BASE			0x39
#define GPIOB_BASE			0x36
#define GPIOC_BASE			0x33
#define GPIOD_BASE			0x30

#define SFIOR				0x50

#define SPI_BASE			0X2D

#define ADC_BASE			0X24

#define UBRRL				(*(volatile uint8_t*)0x29)
#define UCSRB				(*(volatile uint8_t*)0x2A)
#define UCSRA				(*(volatile uint8_t*)0x2B)
#define UDR					(*(volatile uint8_t*)0x2C)
#define UBRRH				(*(volatile uint8_t*)0x40)
#define UCSRC				(*(volatile uint8_t*)0x40)

#define TWBR				(*(volatile uint8_t*)0x20)
#define TWSR				(*(volatile uint8_t*)0x21)
#define TWAR				(*(volatile uint8_t*)0x22)
#define TWDR				(*(volatile uint8_t*)0x23)
#define TWCR				(*(volatile uint8_t*)0x56)

#define OCR0				(*(volatile uint8_t*)0x43)
#define TCNT0				(*(volatile uint8_t*)0x44)
#define TCCR0				(*(volatile uint8_t*)0x45)
#define TIFR				(*(volatile uint8_t*)0x58)
#define TIMSK				(*(volatile uint8_t*)0x59)

//===================================================================================

//-----------------------------------------------------------------------------------
//Peripherals registers
//-----------------------------------------------------------------------------------

//-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
//GPIO register
//-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
typedef struct
{
	volatile uint8_t PIN;
	volatile uint8_t DDR;
	volatile uint8_t PORT;
	
}GPIO_t;

//-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
//SPI register
//-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
typedef struct
{
	volatile uint8_t SPCR;
	volatile uint8_t SPSR;
	volatile uint8_t SPDR;
	
}SPI_t;

//-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
//ADC register
//-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
typedef struct
{
	volatile uint16_t ADCDATA;
	volatile uint8_t ADCSRA;
	volatile uint8_t ADMUX;
	
}ADC_t;

//===================================================================================

//-----------------------------------------------------------------------------------
//Peripheral instants
//-----------------------------------------------------------------------------------

#define GPIOA							((GPIO_t *)GPIOA_BASE)
#define GPIOB							((GPIO_t *)GPIOB_BASE)
#define GPIOC							((GPIO_t *)GPIOC_BASE)
#define GPIOD							((GPIO_t *)GPIOD_BASE)

#define SPI								((SPI_t *)SPI_BASE)

#define ADC								((ADC_t *)ADC_BASE)

//===================================================================================

//-----------------------------------------------------------------------------------
//Enable & disable macros
//-----------------------------------------------------------------------------------

#define PUR_EN								(*(uint8_t*)SFIOR = 0X00)
#define PUR_DIS								(*(uint8_t*)SFIOR = 0X04)

#define SPI_EN								(SPI->SPCR |= 0X40)
#define ADC_EN								(ADC->ADCSRA |= 0X80)
#define ADC_START							(ADC->ADCSRA |= 0X40)
//===================================================================================

#endif /* ATMEGA32A_DEVICE_HEADER_H_ */